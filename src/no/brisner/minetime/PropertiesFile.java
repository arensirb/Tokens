...
public class PropertiesFile {
    ...
    public PropertiesFile(File file) {
        ...
        try {
            ...
            Minetime.log.log(Level.SEVERE, "[Tokens]: Cannot read file " + file.getName());
        } catch (IOException e) {
            Minetime.log.log(Level.SEVERE, "[Tokens]: Cannot create file " + file.getName());
        }
    }
    ...
    public int getInt(String key, Integer defaultValue, String defaultComment) {
        if (map.containsKey(key)) {
            try {
                ...
                Minetime.log.log(Level.WARNING, "[Tokens]: Trying to get Integer from " + key + ": " + map.get(key).value);
                return 0;
            }
        } else {
            map.put(key, new PropertiesEntry(defaultValue.toString(), defaultComment));
    ...
    public long getLong(String key, Long defaultValue, String defaultComment) {
        if (map.containsKey(key)) {
            try {
                ...
                Minetime.log.log(Level.WARNING, "[Tokens]: Trying to get Long from " + key + ": " + map.get(key).value);
                return 0;
            }
        } else {
            map.put(key, new PropertiesEntry(defaultValue.toString(), defaultComment));
    ...
    public double getDouble(String key, Double defaultValue, String defaultComment) {
        if (map.containsKey(key)) {
            try {
                ...
                Minetime.log.log(Level.WARNING, "[Tokens]: Trying to get Double from " + key + ": " + map.get(key).value);
                return 0;
            }
        } else {
            map.put(key, new PropertiesEntry(defaultValue.toString(), defaultComment));
    ...
    public void save() {
        ...
        try {
            ...
            Minetime.log.log(Level.SEVERE, "[Tokens]: IO Exception with file " + file.getName());
        } finally {
            try {
                if (bwriter != null) {
                    bwriter.flush();
                ...
                Minetime.log.log(Level.SEVERE, "[Tokens]: IO Exception with file " + file.getName() + " (on close)");
            }
        }
